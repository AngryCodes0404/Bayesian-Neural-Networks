...
parser = argparse.ArgumentParser(
    description="Train Bayesian Neural Net on MNIST with Variational Inference"
)
parser.add_argument(
    "--model",
    type=str,
...
nsamples = int(
    args.n_samples
)  # How many samples to estimate ELBO with at each iteration
########################################################################################
...
if args.model == "Local_Reparam":
    ...
    net = BBP_Bayes_Net(
        ...
        Nbatches=(NTrainPointsMNIST / batch_size),
        nhid=1200,
        prior_instance=spike_slab_2GMM(
            mu1=0, mu2=0, sigma1=args.prior_sig, sigma2=0.0005, pi=0.75
        ),
    )
else:
...
for i in range(epoch, nb_epochs):
    ...
    kl_cost_train[
        i
    ] /= nb_samples  # Normalise by number of samples in order to get comparable number to the -log like
    ...
    # ---- print
    print(
        "it %d/%d, Jtr_KL = %f, Jtr_pred = %f, err = %f, "
        % (i, nb_epochs, kl_cost_train[i], pred_cost_train[i], err_train[i]),
        end="",
    )
    ...
    if i % nb_its_dev == 0:
        ...
        for j, (x, y) in enumerate(valloader):
            cost, err, probs = net.eval(
                x, y
            )  # This takes the expected weights to save time, not proper inference
...
plt.savefig(
    results_dir + "/pred_cost.png", bbox_extra_artists=(lgd,), bbox_inches="tight"
)
...
for item in (
    ...
    item.set_weight("normal")
plt.savefig(
    results_dir + "/KL_cost.png", bbox_extra_artists=(lgd,), bbox_inches="tight"
)
